...
def detect_collision(game_element_1, game_element_2):
    # Calculate cartesian distance between game elements. Signal a collision if the distance is lower than 20
...
def enforce_infinite_screen():
    # Make game elements move to ("reappear at") the opposite side of the screen when they are about to leave the screen
    if player.xcor() > (SCREEN_WIDTH / 2):
        # If the game_element (e.g., the player's ship) leaves on the right side of the screen, move it back by SCREEN_WIDTH
        player.hideturtle()
        player.forward(-SCREEN_WIDTH)
        player.showturtle()
    if player.xcor() < -(SCREEN_WIDTH / 2):
        player.hideturtle()
        player.forward(-SCREEN_WIDTH)
